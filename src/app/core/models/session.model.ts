...
export interface TrainingSession {
  ...
  // True when this session was generated by replenishing a just-finished
  // one (see SessionsComponent.buildManualReplenishment/buildPlanReplenishment) -
  // an intentional continuation of an ongoing progression cycle. A session
  // without this set is either hand-created from scratch (SessionsComponent.
  // addSession) or a bulk "create sessions from plan" - see
  // seedDoubleProgressionTargets/seedWaveProgressionTarget, which use it
  // (together with trainingPlanId, since a plan's own sessions always
  // continue regardless of how they were created) to decide whether an
  // exercise's tracked rep-cycle progress should carry forward or restart.
