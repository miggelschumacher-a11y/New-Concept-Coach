...
const TRANSLATIONS: Record<Language, Record<string, string>> = {
  en: {
    ...
    'config.incrementSchemeDescription':
      'Double progression: work within a fixed rep range (your "Lower"/"Upper rep range") at a fixed weight until every set reaches the top of the range - then the weight goes up and you start again at the bottom. Example with an 8-10 range and 3 sets in "Add a rep to every set" mode: 3x8 → 3x9 → 3x10 → weight goes up, back to 3x8. Example with an 8-10 range and 4 sets in "Add one total extra rep" mode (the extra rep rotates through the sets one at a time instead of landing on all of them at once): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → weight goes up, back to 4x8. A session only counts as successful once every set meets or beats its own target - miss even one and the next session repeats unchanged.',
  ...
  de: {
    ...
    'config.incrementSchemeDescription':
      'Doppelte Progression: Du trainierst innerhalb eines festen Wiederholungsbereichs (deine „Untere"/„Obere Wiederholungsgrenze") bei gleichbleibendem Gewicht, bis jeder Satz die obere Grenze erreicht hat – dann steigt das Gewicht, und du beginnst wieder an der unteren Grenze. Beispiel mit Bereich 8-10 und 3 Sätzen im Modus „Jedem Satz eine Wiederholung hinzufügen": 3x8 → 3x9 → 3x10 → Gewicht steigt, zurück zu 3x8. Beispiel mit Bereich 8-10 und 4 Sätzen im Modus „Insgesamt eine Wiederholung hinzufügen" (die Extra-Wiederholung wandert reihum durch die Sätze, statt bei allen gleichzeitig dazuzukommen): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → Gewicht steigt, zurück zu 4x8. Eine Einheit zählt nur als geschafft, wenn jeder Satz sein eigenes Ziel erreicht oder übertrifft – verfehlst du auch nur einen, wiederholt sich die nächste Einheit unverändert.',
  ...
  es: {
    ...
    'config.incrementSchemeDescription':
      'Progresión doble: entrenas dentro de un rango de repeticiones fijo (tu «rango inferior»/«rango superior» de repeticiones) con un peso fijo hasta que cada serie alcanza el límite superior; entonces el peso sube y vuelves a empezar desde el límite inferior. Ejemplo con un rango de 8-10 y 3 series en el modo «Añadir una repetición a cada serie»: 3x8 → 3x9 → 3x10 → el peso sube, vuelta a 3x8. Ejemplo con un rango de 8-10 y 4 series en el modo «Añadir una repetición total» (la repetición extra rota entre las series una a la vez, en lugar de añadirse a todas a la vez): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → el peso sube, vuelta a 4x8. Una sesión solo cuenta como lograda si cada serie alcanza o supera su propio objetivo; si fallas aunque sea una, la siguiente sesión se repite sin cambios.',
  ...
  pt: {
    ...
    'config.incrementSchemeDescription':
      'Progressão dupla: você treina dentro de uma faixa de repetições fixa (sua "faixa inferior"/"faixa superior" de repetições) com um peso fixo até que cada série atinja o limite superior; então o peso aumenta e você recomeça no limite inferior. Exemplo com uma faixa de 8-10 e 3 séries no modo "Adicionar uma repetição a cada série": 3x8 → 3x9 → 3x10 → o peso aumenta, de volta a 3x8. Exemplo com uma faixa de 8-10 e 4 séries no modo "Adicionar uma repetição total" (a repetição extra passa por cada série uma de cada vez, em vez de ser adicionada a todas ao mesmo tempo): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → o peso aumenta, de volta a 4x8. Uma sessão só conta como bem-sucedida quando cada série atinge ou supera sua própria meta - se você errar mesmo uma, a próxima sessão se repete sem alterações.',
  ...
  it: {
    ...
    'config.incrementSchemeDescription':
      'Progressione doppia: alleni all\'interno di un intervallo di ripetizioni fisso (il tuo "limite inferiore"/"limite superiore" delle ripetizioni) con un peso fisso finché ogni serie raggiunge il limite superiore; a quel punto il peso aumenta e ricominci dal limite inferiore. Esempio con intervallo 8-10 e 3 serie nella modalità "Aggiungi una ripetizione a ogni serie": 3x8 → 3x9 → 3x10 → il peso aumenta, si torna a 3x8. Esempio con intervallo 8-10 e 4 serie nella modalità "Aggiungi una ripetizione totale" (la ripetizione extra passa una serie alla volta invece di aggiungersi a tutte insieme): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → il peso aumenta, si torna a 4x8. Una sessione conta come riuscita solo se ogni serie raggiunge o supera il proprio obiettivo: se ne manchi anche solo una, la sessione successiva si ripete invariata.',
  ...
  nl: {
    ...
    'config.incrementSchemeDescription':
      'Dubbele progressie: je traint binnen een vast herhalingsbereik (je "onderste"/"bovenste herhalingsbereik") met een vast gewicht totdat elke set de bovengrens bereikt; dan gaat het gewicht omhoog en begin je weer bij de ondergrens. Voorbeeld met bereik 8-10 en 3 sets in de modus "Voeg een herhaling toe aan elke set": 3x8 → 3x9 → 3x10 → gewicht gaat omhoog, terug naar 3x8. Voorbeeld met bereik 8-10 en 4 sets in de modus "Voeg in totaal één herhaling toe" (de extra herhaling gaat om de beurt door de sets in plaats van bij alle sets tegelijk): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → gewicht gaat omhoog, terug naar 4x8. Een sessie telt alleen als geslaagd wanneer elke set zijn eigen doel haalt of overtreft - mis je er ook maar één, dan wordt de volgende sessie ongewijzigd herhaald.',
  ...
  pl: {
    ...
    'config.incrementSchemeDescription':
      'Podwójna progresja: trenujesz w stałym zakresie powtórzeń (twój „dolny"/„górny zakres powtórzeń") przy stałym ciężarze, aż każda seria osiągnie górną granicę; wtedy ciężar rośnie i zaczynasz od nowa od dolnej granicy. Przykład dla zakresu 8-10 i 3 serii w trybie „Dodaj powtórzenie do każdej serii": 3x8 → 3x9 → 3x10 → ciężar rośnie, powrót do 3x8. Przykład dla zakresu 8-10 i 4 serii w trybie „Dodaj jedno powtórzenie łącznie" (dodatkowe powtórzenie krąży po kolei między seriami, zamiast trafiać do wszystkich naraz): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → ciężar rośnie, powrót do 4x8. Sesja liczy się jako udana tylko wtedy, gdy każda seria osiąga lub przekracza własny cel – jeśli choć jednej nie osiągniesz, kolejna sesja powtarza się bez zmian.',
  ...
  ru: {
    ...
    'config.incrementSchemeDescription':
      'Двойная прогрессия: вы тренируетесь в фиксированном диапазоне повторений (ваши «нижняя»/«верхняя граница повторений») с фиксированным весом, пока каждый подход не достигнет верхней границы; после этого вес увеличивается, и вы начинаете заново с нижней границы. Пример для диапазона 8-10 и 3 подходов в режиме «Добавлять повторение к каждому подходу»: 3x8 → 3x9 → 3x10 → вес увеличивается, снова 3x8. Пример для диапазона 8-10 и 4 подходов в режиме «Добавлять одно повторение всего» (дополнительное повторение по очереди переходит между подходами, а не добавляется сразу ко всем): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → вес увеличивается, снова 4x8. Тренировка засчитывается как успешная, только если каждый подход достигает или превышает свою цель — если хотя бы один не выполнен, следующая тренировка повторяется без изменений.',
  ...
  hu: {
    ...
    'config.incrementSchemeDescription':
      'Dupla progresszió: egy rögzített ismétlésszám-tartományon belül (a te „alsó"/„felső ismétlésszám-határod") edzel állandó súllyal, amíg minden szett el nem éri a felső határt; ekkor a súly nő, és újra az alsó határtól kezdesz. Példa 8-10-es tartománnyal és 3 szettel az „Ismétlés hozzáadása minden szetthez" módban: 3x8 → 3x9 → 3x10 → a súly nő, vissza 3x8-ra. Példa 8-10-es tartománnyal és 4 szettel az „Összesen egy ismétlés hozzáadása" módban (az extra ismétlés felváltva vándorol a szettek között, nem mindegyikhez kerül egyszerre): 4x8 → 1x9+3x8 → 2x9+2x8 → 3x9+1x8 → 4x9 → 1x10+3x9 → 2x10+2x9 → 3x10+1x9 → 4x10 → a súly nő, vissza 4x8-ra. Egy edzés csak akkor számít sikeresnek, ha minden szett eléri vagy meghaladja a saját célját – ha akár csak egyet elvétesz, a következő edzés változatlanul megismétlődik.',
